    ...
    public function importCommand(
        ...
        if ($dir) {
            foreach ($iterator as $data) {
                if (!$this->nodeImportExportService->shouldImportRecord($data, $path, $matchChildDocuments)) {
                    continue;
                }
        ...
        foreach ($iterator as $data) {
            ...
            $importedCount++;
            if ($progress && $importedCount % $step === 0) {
                $this->output->progressAdvance($step);
            }
        }
        ...
        if ($count) {
            if ($property) {
                ...
                foreach ($iterable as $node) {
                    $node = $node[0];
                    if ($this->matchTermInProperty($node, $search, $property)) {
                        $count++;
                    }
                }
            } else {
